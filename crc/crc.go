// Package crc implements Quake II's move-command checksum.
//
// A vanilla-protocol server VERIFIES the byte a client puts in front of its
// clc_move block: SV_ExecuteClientMessage recomputes COM_BlockSequenceCRCByte
// over the block and IGNORES THE REST OF THE PACKET when it disagrees, without
// telling anybody.  Q2PRO dropped that check long ago, so a client that makes
// the byte up works there and is simply never moved by Yamagi Quake II, id's
// own q2ded or r1q2 -- it connects, it spawns, it receives frames, and every
// usercmd it sends is discarded.
//
// Ported from id's qcommon/crc.c, which Yamagi carries unchanged.
package crc
...
// chkTable is id's chktbl.  It is declared [1024] and initialised with 960
// values; C zero-fills the remainder and the sequence index reaches them, so
// the tail of zeroes is part of the algorithm rather than an omission here.
var chkTable = [1024]byte{
...
// Block is id's CRC_Block.
func Block(b []byte) uint16 {
...
// BlockSequenceCRCByte is id's COM_BlockSequenceCRCByte: the CRC of at most the
// first 60 bytes of the block, salted with four bytes taken from chkTable at an
// offset derived from the outgoing sequence number, folded together with the
// byte sum.
func BlockSequenceCRCByte(base []byte, sequence int) byte {
	if sequence < 0 {
		return 0
	}
	p := sequence % (len(chkTable) - 4)

	n := len(base)
	if n > 60 {
		n = 60
	}
	chkb := make([]byte, 0, n+4)
	chkb = append(chkb, base[:n]...)
	chkb = append(chkb, chkTable[p], chkTable[p+1], chkTable[p+2], chkTable[p+3])
